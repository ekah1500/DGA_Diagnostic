...
try:
   h2 = max(0, float(input('H2 - Hydrogen (ppm): ')))
   ch4 = max(0, float(input('CH4 - Methane (ppm): ')))
   c2h6 = max(0, float(input('C2H6 - Ethane (ppm): ')))
   c2h4 = max(0, float(input('C2H4 - Ethylene (ppm): ')))
   c2h2 = max(0, float(input('C2H2 - Acetylene (ppm): ')))
   co = max(0, float(input('CO - Carbon Monoxide (ppm): ')))
   co2 = max(0, float(input('CO2 - Carbon Dioxide (ppm): ')))
...
def get_rr(num, den): # zero division handling fxn
    if den > 0:
        return num / den
    elif num == 0:
        return 0.0 # 0/0 -> 0 for Normal Logic (Normal is < 0.1)
    else:
        return 999.0 # High/0 -> Infinity (Arcing indicator)

# --- Calculating Ratios ---

r1 = get_rr(c2h2, c2h4) # R1 = Acetylene / Ethylene
r2 = get_rr(ch4, h2)    # R2 = Methane / Hydrogen
r5 = get_rr(c2h4, c2h6) # R5 = Ethylene / Ethane
...
if tdcg < 50: # Standard "noise floor" for DGA
...
# --- Calculating Percentages relative to TDCG ---
if tdcg > 0:
    p_h2   = (h2 / tdcg) * 100
    p_ch4  = (ch4 / tdcg) * 100
...
# CELLULOSE DEGRADATION (Key Gas: Carbon Monoxide)
elif p_co > 80:
    kg_result = 'Overheating of Insulation Paper [Key Gas: Carbon Monoxide]'

# LOW-TEMP THERMAL (Key Gas: Ethane or Methane)
...
# --- Vertices ---
# Tip of triangle (100% CH4), Right (100% C2H4), Left (100% C2H2)
top   = [0.5, 0.866]
right = [1.0, 0.0]
left  = [0.0, 0.0]

# Standard Duval coordinates (a-o)
...
if gas_sum_dt1 < 1:
    ...
    # Coordinate Transformation (Ternary -> Cartesian)
...
def plot_dt4(h2, ch4, c2h6):
    print('\n---> Duval Triangle 4 (Low Temp Faults) <---')

    # Function for Triangular -> Cartesian
    def triangular_dt4(h, m, e):
        return ((m + 0.5 * h), (h * 0.866025))

    # Polygon Regions (IEEE C57.104-2019 / Duval 4 definitions)
    ...
    # S (Stray Gassing) - The central region
    raw_s = [
        (98, 2, 0), (97, 2, 1), (84, 15, 1), (85, 15, 0), # PD border
        (64, 36, 0), (40, 36, 24), (15, 61, 24), (15, 55, 30), # C border
        (9, 61, 30), (9, 45, 46), (54, 0, 46), (100, 0, 0) # ND border & Top
    ]
...
def plot_dt5(ch4, c2h4, c2h6):
    print('\n---> Duval Triangle 5 (High Temp Faults) <---')

    # fxn for coords
    # Gas (C2H4, C2H6) -> Cartesian (X, Y)
    def get_dt5_coords(c2h4, ch4):
        return (c2h4 + 0.5 * ch4, ch4 * 0.866025)

    # fxn to convert list of (C2H4, C2H6) tuples to Cartesian Polygons
    ...
    # --- Zones ---
    add_patch(poly_t3, '#73a6ff') # T3
    add_patch(poly_c,  '#ffae1a') # C
    add_patch(poly_nd, '#cacdf5') # ND
    add_patch(poly_t2, '#337dff') # T2
    add_patch(poly_s,  '#ff97a9') # S
    add_patch(poly_pd, '#ffff1a') # PD
    add_patch(poly_o_top, '#ff1a1a') # O Top
    add_patch(poly_o_bot, '#ff1a1a') # O Bot

    # --- Triangle Boundary ---
    # Vertices: Left(0,0), Right(100,0), Top(50, 86.6)
    boundary = [(0,0), (100,0), (50, 86.6025), (0,0)]
    bx, by = zip(*boundary)
    ax.plot(bx, by, 'k-', linewidth=1.5)

    # --- Text Labels ---
    h = 100 * math.sqrt(3) / 2  # ~86.6
    ...
    add_lbl(46, h-8, 'PD')
    add_lbl(50, h-10, 'O', 'black') # Top O
    add_lbl(33, 50,  'S')
    add_lbl(15, 20,  'O', 'black') # Bot O
    add_lbl(35, 20,  'ND')
    add_lbl(55, 40,  'C')
    add_lbl(52, 11,  'T3')
    add_lbl(83, 15,  'T3')
...
input("\nAnalysis Complete. Press Enter to close charts and exit...")